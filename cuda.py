...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, n_var, action_std):      #state = 2+2+1+240 = 245
        ...
        ######### Actor Layers #########
        self.lstm1_a = nn.LSTM(input_size=3,hidden_size=63).to(device)
        self.fc2_a = nn.Linear(64,64).to(device)
        self.fc3_a = nn.Linear(64,2).to(device)
        self.h0_a = torch.zeros(1,1,63).to(device)
        self.c0_a = torch.zeros(1,1,63) .to(device)                   
                

        ######### Critic Layers #########
        self.lstm1_c = nn.LSTM(input_size=3,hidden_size=63).to(device)
        self.fc2_c = nn.Linear(64,64).to(device)
        self.fc3_c = nn.Linear(64,1).to(device)
        self.h0_c = torch.zeros(1,1,63).to(device)
        self.c0_c = torch.zeros(1,1,63).to(device)
    ...
    def actor(self,x,y):
        x = x.view(-1,1,3) #torch.FloatTensor(np.array(x)).view(-1,1,3)
        x, _ = self.lstm1_a(x,(self.h0_a,self.c0_a))
        x = x[-1,:,:]
        x = x.view(-1)
        x = torch.tanh(x)
           
        z = torch.cat((x,y),dim=-1) #z = torch.cat((x,torch.FloatTensor(np.array([y]))),dim=-1)
        z = self.fc2_a(z)
        z = torch.tanh(z)
        z = self.fc3_a(z)
        z = torch.tanh(z)
    ...
    def evaluate(self, state, action):
        ...
        for idx_1 in range(no_vehicles):
            action_mean.append(self.actor(torch.FloatTensor(np.array(state[idx_1][1])).to(device),torch.FloatTensor(np.array([state[idx_1][0]])).to(device))) #action_mean.append(self.actor(state[idx_1][1],state[idx_1][0]))
            state_value.append(self.critic(torch.FloatTensor(np.array(state[idx_1][1])).to(device),torch.FloatTensor(np.array([state[idx_1][0]])).to(device))) #state_value.append(self.critic(state[idx_1][1],state[idx_1][0]))
...
class AgentHandler:
    ...
    def select_action(self,state,actor):
        ...
        for idx_1 in range(no_vehicles):
            ...
            action_mean = actor(torch.FloatTensor(np.array(other_vehicles1)).to(device),torch.FloatTensor(np.array([target1])).to(device)) #1
...
def main():
    ...
    save_interval = 1000              # Interval model is saved
