...
class ActorCritic(nn.Module):  #所有模型的父类，自定义模型通过重写该模型来实现
    ...
    def evaluate(self, state, action):
        ...
        return action_logprobs, torch.squeeze(state_value), dist_entropy  #多返回值函数
...
class PPO:
    ...
    def update(self, memory):
        ...
        for _ in range(self.K_epochs):
            ...
            # Finding Surrogate Loss: #替代损失函数，如交叉熵
...
class AgentHandler:
    ...
    def select_action(self,state,actor):
        self.action_var = torch.full((2,), 0.6*0.6).to(device)   #manually change action_dim action_std 手动更改空间维度和每个维度上的标准差
        no_vehicles = len(state)
        action_list = []
        for idx_1 in range(no_vehicles):
            state1 = state[idx_1]
